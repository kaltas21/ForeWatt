...
from src.models.new_experiment.deeplearning.hardware_config import get_hardware_config
...
class PatchTSTTrainer:
    ...
    def train(
        ...
        print(f"\n{'='*70}")
        print(f"  PATCHTST TRAINING: {self.target}")
        print(f"{'='*70}")
        print(f"  Device: {self.device_type.upper()}")
        print(f"  Horizon: {self.horizon}h, Input: {self.input_size}h")
        print(f"  Config: patch_len={hyperparams['patch_len']}, stride={hyperparams['stride']}")
        print(f"  Config: layers={hyperparams.get('encoder_layers', hyperparams.get('n_layers', 3))}, hidden={hyperparams['hidden_size']}, heads={hyperparams['n_heads']}")
