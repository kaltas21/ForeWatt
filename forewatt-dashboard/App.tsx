...
import {
    ...
    RefreshCw, Mail, Smartphone, Shield, User
} from 'lucide-react';
...
const AppContent = () => {
  ...
  // Auth view state (login or signup)
  const [authView, setAuthView] = useState<AuthView>('signup'); // Default to signup for new users
  ...
  const handleLogout = () => {
    ...
    setView('REALTIME');
    setSidebarOpen(true);
    setAuthView('login'); // Go to login after logout
  };
      ...
      >
          ...
          <div className="p-6 border-t border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-3">
                 <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isAdmin ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-slate-200 dark:bg-slate-700'}`}>
                    {isAdmin ? <Shield size={16} className="text-amber-600 dark:text-amber-400" /> : <User size={16} className="text-slate-500 dark:text-slate-300" />}
                 </div>
                 <div>
                     <div className="text-sm font-medium text-slate-900 dark:text-white">{user?.displayName}</div>
                     <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
                       {isAdmin ? (
                         <><Shield size={10} className="text-amber-500" /> Admin</>
                       ) : (
                         <><User size={10} /> Client</>
                       )}
                     </div>
                ...
                <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                    ...
                    <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div className="space-y-4">
                            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Account</h4>
                            ...
                            <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-lg ${isAdmin ? 'bg-amber-500/10 text-amber-500' : 'bg-primary-500/10 text-primary-500'}`}>
                                ...
                                <span className={`text-xs font-bold px-2 py-1 rounded-full ${isAdmin ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' : 'bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400'}`}>
                        ...
                        <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                         ...
                         {/* Section: Notifications (Admin only) */}
                         {isAdmin && (
                           <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
